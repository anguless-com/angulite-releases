<!--
  SOURCE OF TRUTH for the PUBLIC mirror repo README (anguless-com/angulite-releases).
  Edit here, then publish to the mirror. Keeping it version-controlled here means the
  download page can't silently drift from what actually shipped.

  Per release, update: (1) the version + date, (2) the download table's SHA256 values,
  (3) the Changelog section (excerpt from CHANGELOG.md).

  Compute checksums from the PUBLISHED artifacts, never from a local build:
    gh release download vX.Y.Z --repo anguless-com/angulite-releases \
      --pattern "*.dmg" --pattern "*-setup.exe"
    sha256sum AnguLite_*.dmg AnguLite_*-setup.exe

  PUBLISHED WHOLE 2026-07-29 as mirror commit 2d96a40 (README 8479 -> 10830 bytes).
  No drift: the live page and this file are the same content. The earlier TRIMMED
  variant (c9c5843, 2026-07-28) is superseded.

  The four screenshots live in the mirror at docs/screenshots/*.png (commits 6799c27,
  e77eebc, 85c1189, 1b02531). Image paths in this file are relative to the MIRROR, so
  if you ever re-publish, push changed images BEFORE the README or the page renders
  broken images in the gap.
...
d36c56c2d500700262a3ce0d54c7df4c1578184927e43b2af2aabf4a50be4c8d  AnguLite_0.3.4_aarch64.dmg
2f69a426a0029295546d178fd92d9a1fb86dc5717a6585a0def242e4c2cb525c  AnguLite_0.3.4_x64.dmg
```
